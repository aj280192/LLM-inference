...
# ADDENDUM 2: Logging, complete skeleton & file inventory

## Colored logs & summaries in GitLab job output

All bash scripts source `lib/log.sh`, which gives every job:
- Colored `[INFO]` (blue), `[ OK ]` (green), `[WARN]` (yellow), `[FAIL]` (red) lines
- **Collapsible sections** in the GitLab job viewer (each phase — vault login,
  per-host deploy, health check, manifest — folds under a clickable header)
- An automatic **SUMMARY table at the end of every script** listing each
  recorded step with its status and a final green SUCCESS / red FAILED verdict —
  so the bottom of a job log answers "what worked, what didn't" at a glance
- `FF_SCRIPT_SECTIONS: "true"` in the pipeline additionally folds each raw
  script command into its own section

Example of what a failing deploy's tail looks like:
```
[10:02:11] [ OK ] Vault login
[10:02:13] [ OK ] pinned secret version: secret/chatapp/poc v7
[10:02:31] [FAIL] deploy FAILED on poc-server — aborting rollout
============ SUMMARY: deploy_from_buildhost.sh (24s) ============
  [OK  ] Vault login (token TTL short-lived, revoked on exit)
  [OK  ] pinned secret version: secret/chatapp/poc v7
  [FAIL] deploy FAILED on poc-server — aborting rollout
  RESULT: FAILED (exit 1)  (2 ok, 0 warn, 1 fail)
```

`deploy_local.sh` needs the library on the deploy host too — `setup_poc_host.sh`
now installs it at `/opt/chatapp/lib/log.sh` automatically.

## No external cloud dependencies

Everything here uses only: GitLab (+runners), internal Artifactory
(docker + pip), Vault, docker, nginx, ssh. No S3/AWS/GCP anywhere — release
tracking is a git repo, host state is local files, images live in Artifactory.

## Complete file inventory

```
.gitlab-ci.yml                     THE pipeline (dotted file! see SETUP.md)
gitlab-ci.yml.REFERENCE-COPY.yml   identical visible copy (reading only)
SETUP.md                           quick setup order
Dockerfile                         proxy + internal-pip aware, BuildKit secret
requirements.txt / requirements-dev.txt
src/main.py                        minimal FastAPI app skeleton (/health, /chat)
prompts/*.j2                       prompt templates (validated in CI)
tests/unit/                        pytest skeleton
lib/log.sh                         shared colored logging + summaries
ci/deploy_from_buildhost.sh        deploy orchestrator (vault pin -> ssh -> manifest)
ci/rollback_from_buildhost.sh      paired rollback (secret first, then image)
deploy/deploy_local.sh             on-host blue-green deploy
deploy/nginx-upstream.conf.tpl     traffic-swap template
vault/vault_setup.sh               policies + AppRoles incl. poc-only role
vault/vault-agent.hcl              agent as token sink
scripts/setup_build_host.sh        runner-as-service-account, docker/pip proxy
scripts/setup_poc_host.sh          deploy user, agent service, scripts, nginx
scripts/rotate_secret.sh           the only sanctioned secret-change path
scripts/validate_prompts.py        CI prompt check
eval/run_eval.py                   eval gate (mocked or live-target)
eval/golden_set.jsonl              MR-gate eval cases
eval/smoke_set.jsonl               post-deploy live smoke cases
eval/full_set.jsonl                nightly full set (grow this over time)
```
